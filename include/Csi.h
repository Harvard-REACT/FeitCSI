...
class Csi {
   ...
   private:
    ...
    const std::vector<uint32_t> HT_VHT_40_PILOT_INDICES = {5,  33, 47,
                                                           66, 80, 108};  // 114 subcarriers
    const std::vector<uint32_t> VHT_80_PILOT_INDICES = {19,  47,  83,  111,
                                                        130, 158, 194, 222};  // 242 subcarriers
    const std::vector<uint32_t> VHT_160_PILOT_INDICES = {19,  47,  83,  111, 130, 158,
                                                         194, 222, 261, 289, 325, 353,
                                                         372, 400, 436, 464};  // 484 subcarriers
    const std::vector<uint32_t> HE_20_PILOT_INDICES = {6,   32,  74,  100,
                                                       141, 167, 209, 235};  // 242 subcarriers
    const std::vector<uint32_t> HE_40_PILOT_INDICES = {
        6,   32,  74,  100, 140, 166, 208, 234,
        249, 275, 317, 343, 383, 409, 451, 477};  // 484 subcarriers
    const std::vector<uint32_t> HE_80_PILOT_INDICES = {
        32,  100, 166, 234, 274, 342, 408, 476,
        519, 587, 653, 721, 761, 829, 895, 963};  // 996 subcarriers
    const std::vector<uint32_t> HE_160_PILOT_INDICES = {
        32,   100,  166,  234,  274,  342,  408,  476,  519,  587,  653,
        721,  761,  829,  895,  963,  1028, 1096, 1162, 1230, 1270, 1338,
        1404, 1472, 1515, 1583, 1649, 1717, 1757, 1825, 1891, 1959};  // 1992 subcarriers
