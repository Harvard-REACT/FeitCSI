...
uint8_t beaconHeader[] = {0x80, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xea, 0xb9,
                          0xcc, 0xc2, 0x4a, 0xaf, 0xea, 0xb9, 0xcc, 0xc2, 0x4a, 0xaf, 0x00, 0x00,
                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x64, 0x00, 0x01, 0x04,
                          0x00, 0x07, 0x46, 0x65, 0x69, 0x74, 0x43, 0x53, 0x49, 0x01, 0x08, 0x82,
                          0x84, 0x8b, 0x96, 0x0c, 0x12, 0x18, 0x24, 0x03, 0x01, 0x01};
...
int WiFiFtmController::startInitiator() {
    Cmd cmd{
        ...
        .pre_execute_handler = this->ftmHandler,
...
int WiFiFtmController::startResponder() {
    Cmd cmd{
        ...
        .nlFlags = 0,
        .device = if_nametoindex(AP_INTERFACE_NAME),
        .pre_execute_handler = this->ftmResponderHandler,
        .valid_handler = NULL,
    };
...
int WiFiFtmController::setApMode() {
    Cmd cmd{
        ...
        .nlFlags = 0,
        .device = if_nametoindex(AP_INTERFACE_NAME),
        .pre_execute_handler = this->setApModeHandler,
        .valid_handler = NULL,
    };
...
int WiFiFtmController::ftmResponderHandler(struct nl80211_state* state,
                                           struct nl_msg* msg,
                                           void* arg) {
...
int WiFiFtmController::setApModeHandler(struct nl80211_state* state,
                                        struct nl_msg* msg,
                                        void* arg) {
...
int WiFiFtmController::processFtmHandler(struct nl_msg* msg, void* arg) {
    ...
    if (gnlh->cmd == NL80211_CMD_PEER_MEASUREMENT_RESULT) {
        ...
        nla_parse_nested(resp, NL80211_PMSR_RESP_ATTR_MAX, tbpeer[NL80211_PMSR_PEER_ATTR_RESP],
                         NULL);
        ...
        Ftm ftmData{
            .burstIndex = (uint32_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_BURST_INDEX]
                                         ? nla_get_u32(ftm[NL80211_PMSR_FTM_RESP_ATTR_BURST_INDEX])
                                         : 0),
            .numFtmrAttempts =
                (uint32_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_NUM_FTMR_ATTEMPTS]
                               ? nla_get_u32(ftm[NL80211_PMSR_FTM_RESP_ATTR_NUM_FTMR_ATTEMPTS])
                               : 0),
            .numFtmrSuccesses =
                (uint32_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_NUM_FTMR_SUCCESSES]
                               ? nla_get_u32(ftm[NL80211_PMSR_FTM_RESP_ATTR_NUM_FTMR_SUCCESSES])
                               : 0),
            .numBurstsExp =
                (uint32_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_NUM_BURSTS_EXP]
                               ...
                               : 0),
            .burstDuration =
                (uint8_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_BURST_DURATION]
                              ? nla_get_u8(ftm[NL80211_PMSR_FTM_RESP_ATTR_BURST_DURATION])
                              : 0),
            .ftmsPerBurst =
                (uint8_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_FTMS_PER_BURST]
                              ? nla_get_u8(ftm[NL80211_PMSR_FTM_RESP_ATTR_FTMS_PER_BURST])
                              : 0),
            .rssiAvg = (uint8_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_RSSI_AVG]
                                     ? nla_get_u8(ftm[NL80211_PMSR_FTM_RESP_ATTR_RSSI_AVG])
                                     : 0),
            .rssiSpread = (uint32_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_RSSI_SPREAD]
                                         ? nla_get_u32(ftm[NL80211_PMSR_FTM_RESP_ATTR_RSSI_SPREAD])
                                         : 0),
            .rttAvg = (uint64_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_RTT_AVG]
                                     ? nla_get_u64(ftm[NL80211_PMSR_FTM_RESP_ATTR_RTT_AVG])
                                     : 0),
            .rttVariance =
                (uint64_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_RTT_VARIANCE]
                               ? nla_get_u64(ftm[NL80211_PMSR_FTM_RESP_ATTR_RTT_VARIANCE])
                               : 0),
            .rttSpread = (uint64_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_RTT_SPREAD]
                                        ? nla_get_u64(ftm[NL80211_PMSR_FTM_RESP_ATTR_RTT_SPREAD])
                                        : 0),
            .distAvg = (uint64_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_DIST_AVG]
                                      ? nla_get_u64(ftm[NL80211_PMSR_FTM_RESP_ATTR_RTT_AVG])
                                      : 0),
            .distVariance =
                (uint64_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_DIST_VARIANCE]
                               ? nla_get_u64(ftm[NL80211_PMSR_FTM_RESP_ATTR_DIST_VARIANCE])
                               : 0),
            .distSpread = (uint64_t)(ftm[NL80211_PMSR_FTM_RESP_ATTR_DIST_SPREAD]
        ...
        if (MainController::getInstance()->udpSocket) {
            MainController::getInstance()->udpSocket->send(reinterpret_cast<char*>(&ftmData),
                                                           FTM_SIZE);
        } else {
